...
class CacheExample {
  /// 基础数据类型存储示例
  static Future<void> basicDataExample() async {
    print('=== 基础数据类型存储示例 ===');
  ...
  /// JSON数据存储示例
  static Future<void> jsonDataExample() async {
    print('\n=== JSON数据存储示例 ===');
  ...
  /// 过期时间管理示例
  static Future<void> expireTimeExample() async {
    print('\n=== 过期时间管理示例 ===');

    // 存储带过期时间的数据（1小时后过期）
    await CacheUtils.setString('token', 'abc123xyz',
  ...
  /// 缓存管理示例
  static Future<void> cacheManagementExample() async {
    print('\n=== 缓存管理示例 ===');
  ...
  /// 对象存储示例
  static Future<void> objectStorageExample() async {
    print('\n=== 对象存储示例 ===');
    ...
    await CacheUtils.setObject('currentUser', user,
        expireDuration: Duration(hours: 2));

    // 获取用户对象
    Map<String, dynamic>? cachedUser =
        await CacheUtils.getObject('currentUser', (json) => json);
