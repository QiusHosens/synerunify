...
use crate::convert::erp_purchase_return_detail::{create_request_to_model, update_request_to_model, model_to_response};
...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateErpPurchaseReturnDetailRequest) -> Result<()> {
    let erp_purchase_return_detail = ErpPurchaseReturnDetailEntity::find_by_id(request.id)
        .filter(Column::TenantId.eq(login_user.tenant_id))
        .one(db)
        .await?
        .ok_or_else(|| anyhow!("记录未找到"))?;

    let mut erp_purchase_return_detail = update_request_to_model(&request, erp_purchase_return_detail);
    erp_purchase_return_detail.updater = Set(Some(login_user.id));
