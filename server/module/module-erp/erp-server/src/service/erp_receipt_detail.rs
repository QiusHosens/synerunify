...
use crate::convert::erp_receipt_detail::{create_request_to_model, update_request_to_model, model_to_response};
...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateErpReceiptDetailRequest) -> Result<()> {
    let erp_receipt_detail = ErpReceiptDetailEntity::find_by_id(request.id)
        .one(db)
        .await?
        .ok_or_else(|| anyhow!("记录未找到"))?;

    let mut erp_receipt_detail = update_request_to_model(&request, erp_receipt_detail);
    erp_receipt_detail.updater = Set(Some(login_user.id));
