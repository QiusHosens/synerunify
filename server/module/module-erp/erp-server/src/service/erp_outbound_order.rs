...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateErpOutboundOrderRequest) -> Result<()> {
    let erp_outbound_order = ErpOutboundOrderEntity::find_by_id(request.id)
    ...
    let mut erp_outbound_order = update_request_to_model(&request, erp_outbound_order);
...
pub async fn get_paginated(db: &DatabaseConnection, login_user: LoginUserContext, params: PaginatedKeywordRequest) -> Result<PaginatedResponse<ErpOutboundOrderResponse>> {
    let condition = Condition::all().add(Column::TenantId.eq(login_user.tenant_id));let paginator = ErpOutboundOrderEntity::find_active_with_condition(condition)
