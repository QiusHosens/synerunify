...
use crate::convert::erp_receipt_attachment::{create_request_to_model, model_to_base_response, model_to_response, update_add_request_to_model, update_request_to_model};
...
pub async fn create_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, order_id: i64, requests: Vec<CreateErpReceiptAttachmentRequest>) -> Result<()> {
    let file_ids: Vec<i64> = requests.iter().clone().map(|request| request.file_id).collect();

    let models: Vec<ErpReceiptAttachmentActiveModel> = requests
        .into_iter()
        .map(|request| {
    ...
    if !models.is_empty() {
        ErpReceiptAttachmentEntity::insert_many(models)
            .exec(txn)
            .await
...
pub async fn update_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, order: ErpReceiptModel, requests: Vec<UpdateErpReceiptAttachmentRequest>) -> Result<()> {
    // 查询已存在订单附件
    let existing_attachments = ErpReceiptAttachmentEntity::find()
    ...
    // 构造 HashMap 加速查询
    let existing_map: HashMap<i64, ErpReceiptAttachmentModel> =
    ...
    if !to_add.is_empty() {
        ...
        let models: Vec<ErpReceiptAttachmentActiveModel> = to_add
            .into_iter()
            .map(|request| {
    ...
    if !to_mark_deleted.is_empty() {
        ErpReceiptAttachmentEntity::update_many()
...
pub async fn list_by_order_id(db: &DatabaseConnection, login_user: LoginUserContext, order_id: i64) -> Result<Vec<ErpReceiptAttachmentBaseResponse>> {
    let list = ErpReceiptAttachmentEntity::find_active()
        .filter(Column::TenantId.eq(login_user.tenant_id))
    ...
    let result: Vec<ErpReceiptAttachmentBaseResponse> = list
        .into_iter()
        .map(|a| {
