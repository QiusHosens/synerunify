...
pub async fn update_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, purchase_order: ErpPurchaseOrderModel, requests: Vec<UpdateErpPurchaseOrderAttachmentRequest>) -> Result<()> {
    // 查询已存在订单附件
    let existing_attachments = ErpPurchaseOrderAttachmentEntity::find()
    ...
    // 构造 HashMap 加速查询
    let existing_map: HashMap<i64, ErpPurchaseOrderAttachmentModel> =
    ...
    if !to_add.is_empty() {
        ...
        let models: Vec<ErpPurchaseOrderAttachmentActiveModel> = to_add
            .into_iter()
            .map(|request| {
    ...
    if !to_mark_deleted.is_empty() {
        ErpPurchaseOrderAttachmentEntity::update_many()
...
pub async fn list_by_purchase_id(db: &DatabaseConnection, login_user: LoginUserContext, purchase_id: i64) -> Result<Vec<ErpPurchaseOrderAttachmentBaseResponse>> {
    ...
    let result: Vec<ErpPurchaseOrderAttachmentBaseResponse> = list
        .into_iter()
        .map(|a| {
