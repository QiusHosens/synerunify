...
pub async fn list_by_supplier_id(db: &DatabaseConnection, login_user: LoginUserContext, supplier_id: i64) -> Result<Vec<ErpInboundOrderResponse>> {
    let list = ErpInboundOrderEntity::find_active()
        .filter(Column::TenantId.eq(login_user.tenant_id))
