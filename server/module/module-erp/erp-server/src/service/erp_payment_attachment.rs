...
pub async fn create(db: &DatabaseConnection, login_user: LoginUserContext, request: CreateErpPaymentAttachmentRequest) -> Result<i64> {
    let mut erp_payment_attachment = create_request_to_model(&request);
    erp_payment_attachment.creator = Set(Some(login_user.id));
    erp_payment_attachment.updater = Set(Some(login_user.id));
    erp_payment_attachment.tenant_id = Set(login_user.tenant_id);
    let erp_payment_attachment = erp_payment_attachment.insert(db).await?;
...
pub async fn delete(db: &DatabaseConnection, login_user: LoginUserContext, id: i64) -> Result<()> {
    let erp_payment_attachment = ErpPaymentAttachmentActiveModel {
        id: Set(id),
...
pub async fn get_by_id(db: &DatabaseConnection, login_user: LoginUserContext, id: i64) -> Result<Option<ErpPaymentAttachmentResponse>> {
    let condition = Condition::all()
            .add(Column::Id.eq(id))
            .add(Column::TenantId.eq(login_user.tenant_id));
            
    let erp_payment_attachment = ErpPaymentAttachmentEntity::find_active_with_condition(condition)
        .one(db).await?;
    Ok(erp_payment_attachment.map(model_to_response))
}

pub async fn get_paginated(db: &DatabaseConnection, login_user: LoginUserContext, params: PaginatedKeywordRequest) -> Result<PaginatedResponse<ErpPaymentAttachmentResponse>> {
    let condition = Condition::all().add(Column::TenantId.eq(login_user.tenant_id));let paginator = ErpPaymentAttachmentEntity::find_active_with_condition(condition)
...
pub async fn list(db: &DatabaseConnection, login_user: LoginUserContext) -> Result<Vec<ErpPaymentAttachmentResponse>> {
