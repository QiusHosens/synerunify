...
use crate::convert::erp_inbound_order_attachment::{create_request_to_model, model_to_base_response, model_to_info_response, model_to_response, update_add_request_to_model, update_request_to_model};
...
pub async fn create_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, order_id: i64, requests: Vec<CreateErpInboundOrderAttachmentRequest>) -> Result<()> {
    let file_ids: Vec<i64> = requests.iter().clone().map(|request| request.file_id).collect();

    let models: Vec<ErpInboundOrderAttachmentActiveModel> = requests
        .into_iter()
        .map(|request| {
    ...
    if !models.is_empty() {
        ErpInboundOrderAttachmentEntity::insert_many(models)
            .exec(txn)
            .await
...
pub async fn update_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, order: ErpInboundOrderModel, requests: Vec<UpdateErpInboundOrderAttachmentRequest>) -> Result<()> {
    // 查询已存在订单附件
    let existing_attachments = ErpInboundOrderAttachmentEntity::find()
    ...
    // 构造 HashMap 加速查询
    let existing_map: HashMap<i64, ErpInboundOrderAttachmentModel> =
    ...
    if !to_add.is_empty() {
        ...
        let models: Vec<ErpInboundOrderAttachmentActiveModel> = to_add
            .into_iter()
            .map(|request| {
    ...
    if !to_mark_deleted.is_empty() {
        ErpInboundOrderAttachmentEntity::update_many()
...
pub async fn list_by_inbound_id(db: &DatabaseConnection, login_user: LoginUserContext, inbound_id: i64) -> Result<Vec<ErpInboundOrderAttachmentBaseResponse>> {
    let list = ErpInboundOrderAttachmentEntity::find_active()
        .filter(Column::TenantId.eq(login_user.tenant_id))
    ...
    let result: Vec<ErpInboundOrderAttachmentBaseResponse> = list
        .into_iter()
        .map(|a| {
...
pub async fn list_info_by_inbound_id(db: &DatabaseConnection, login_user: LoginUserContext, inbound_id: i64) -> Result<Vec<ErpInboundOrderAttachmentInfoResponse>> {
    let list = ErpInboundOrderAttachmentEntity::find_active()
        .filter(Column::TenantId.eq(login_user.tenant_id))
        .filter(Column::OrderId.eq(inbound_id))
        .all(db).await?;
    ...
    let files = system_file::list_by_ids(&db, login_user, file_ids).await?;
    let file_map: HashMap<i64, SystemFileModel> = files.iter().map(|f|(f.id, f.clone())).collect();
    let result: Vec<ErpInboundOrderAttachmentInfoResponse> = list
        .into_iter()
        .map(|a| {
            let file_name = file_map
                .get(&a.file_id)
