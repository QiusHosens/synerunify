...
use crate::convert::erp_inventory_transfer_attachment::{create_request_to_model, model_to_base_response, model_to_response, update_add_request_to_model, update_request_to_model};
...
pub async fn create_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, order_id: i64, requests: Vec<CreateErpInventoryTransferAttachmentRequest>) -> Result<()> {
    let file_ids: Vec<i64> = requests.iter().clone().map(|request| request.file_id).collect();

    let models: Vec<ErpInventoryTransferAttachmentActiveModel> = requests
        .into_iter()
        .map(|request| {
    ...
    if !models.is_empty() {
        ErpInventoryTransferAttachmentEntity::insert_many(models)
            .exec(txn)
            .await
...
pub async fn update_batch(db: &DatabaseConnection, txn: &DatabaseTransaction, login_user: LoginUserContext, order: ErpInventoryTransferModel, requests: Vec<UpdateErpInventoryTransferAttachmentRequest>) -> Result<()> {
    // 查询已存在订单附件
    let existing_attachments = ErpInventoryTransferAttachmentEntity::find()
    ...
    // 构造 HashMap 加速查询
    let existing_map: HashMap<i64, ErpInventoryTransferAttachmentModel> =
    ...
    if !to_add.is_empty() {
        ...
        let models: Vec<ErpInventoryTransferAttachmentActiveModel> = to_add
            .into_iter()
            .map(|request| {
    ...
    if !to_mark_deleted.is_empty() {
        ErpInventoryTransferAttachmentEntity::update_many()
...
pub async fn list_by_order_id(db: &DatabaseConnection, login_user: LoginUserContext, order_id: i64) -> Result<Vec<ErpInventoryTransferAttachmentBaseResponse>> {
    let list = ErpInventoryTransferAttachmentEntity::find_active()
        .filter(Column::TenantId.eq(login_user.tenant_id))
    ...
    let result: Vec<ErpInventoryTransferAttachmentBaseResponse> = list
        .into_iter()
        .map(|a| {
