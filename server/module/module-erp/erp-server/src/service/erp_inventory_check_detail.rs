...
use crate::convert::erp_inventory_check_detail::{create_request_to_model, update_request_to_model, model_to_response};
...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateErpInventoryCheckDetailRequest) -> Result<()> {
    let erp_inventory_check_detail = ErpInventoryCheckDetailEntity::find_by_id(request.id)
        .filter(Column::TenantId.eq(login_user.tenant_id))
        .one(db)
        .await?
        .ok_or_else(|| anyhow!("记录未找到"))?;

    let mut erp_inventory_check_detail = update_request_to_model(&request, erp_inventory_check_detail);
    erp_inventory_check_detail.updater = Set(Some(login_user.id));
