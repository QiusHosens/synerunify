...
pub fn model_to_info_other_response(model: ErpInboundOrder, model_supplier: Option<ErpSupplier>, model_settlement_account: Option<ErpSettlementAccount>, details: Vec<ErpInboundOrderDetailBaseOtherResponse>, attachments: Vec<ErpInboundOrderAttachmentBaseResponse>) -> ErpInboundOrderInfoOtherResponse {
    let supplier_name = model_supplier.map(|supplier| supplier.name.clone());
    let settlement_account_name = model_settlement_account.map(|settlement_account| settlement_account.name.clone());

    ErpInboundOrderInfoOtherResponse { 
        id: model.id,
        order_number: model.order_number,
        purchase_id: model.purchase_id,
        supplier_id: model.supplier_id,
