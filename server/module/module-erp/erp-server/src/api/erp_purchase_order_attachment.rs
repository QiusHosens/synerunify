...
#[utoipa::path(
    post,
    path = "/update",
    operation_id = "erp_purchase_order_attachment_update",
    request_body(content = UpdateErpPurchaseOrderAttachmentRequest, description = "update", content_type = "application/json"),
    responses(
...
#[require_authorize(operation_id = "erp_purchase_order_attachment_update", authorize = "")]
async fn update(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<UpdateErpPurchaseOrderAttachmentRequest>,
) -> CommonResult<()> {
    match service::erp_purchase_order_attachment::update(&state.db, login_user, payload).await {
