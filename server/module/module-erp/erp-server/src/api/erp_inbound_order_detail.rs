...
#[utoipa::path(
    post,
    path = "/update",
    operation_id = "erp_inbound_order_detail_update",
    request_body(content = UpdateErpInboundOrderDetailRequest, description = "update", content_type = "application/json"),
    responses(
...
#[require_authorize(operation_id = "erp_inbound_order_detail_update", authorize = "")]
async fn update(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<UpdateErpInboundOrderDetailRequest>,
) -> CommonResult<()> {
    match service::erp_inbound_order_detail::update(&state.db, login_user, payload).await {
