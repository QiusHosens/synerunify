...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "get page", body = CommonResult<PaginatedResponse<ErpProductInventoryResponse>>)
    ),
    tag = "erp_product_inventory",
    security(
        ("bearerAuth" = [])
...
) -> CommonResult<PaginatedResponse<ErpProductInventoryResponse>> {
