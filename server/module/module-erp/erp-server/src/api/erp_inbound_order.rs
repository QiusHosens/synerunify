...
pub async fn erp_inbound_order_router(state: AppState) -> OpenApiRouter {
    OpenApiRouter::new()
        ...
        .routes(routes!(get_info_purchase_by_id))
...
#[utoipa::path(
    get,
    path = "/get_info_purchase/{id}",
    operation_id = "erp_inbound_order_get_info_purchase_by_id",
    params(
        ("id" = i64, Path, description = "id")
    ),
    responses(
        (status = 200, description = "get info purchase by id", body = CommonResult<ErpInboundOrderInfoPurchaseResponse>)
    ),
    tag = "erp_inbound_order",
    security(
        ("bearerAuth" = [])
...
#[require_authorize(operation_id = "erp_inbound_order_getinfo_purchase_by_id", authorize = "")]
async fn get_info_purchase_by_id(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<ErpInboundOrderInfoPurchaseResponse> {
    match service::erp_inbound_order::get_info_purchase_by_id(&state.db, login_user, id).await {
        Ok(Some(data)) => {CommonResult::with_data(data)}
        Ok(None) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
...
#[utoipa::path(
    ...
    operation_id = "erp_inbound_order_get_info_other_by_id",
    params(
        ("id" = i64, Path, description = "id")
    ),
    responses(
        (status = 200, description = "get info other by id", body = CommonResult<ErpInboundOrderInfoOtherResponse>)
    ),
    tag = "erp_inbound_order",
    security(
        ("bearerAuth" = [])
...
#[require_authorize(operation_id = "erp_inbound_order_get_info_other_by_id", authorize = "")]
async fn get_info_other_by_id(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<ErpInboundOrderInfoOtherResponse> {
    match service::erp_inbound_order::get_info_other_by_id(&state.db, login_user, id).await {
        Ok(Some(data)) => {CommonResult::with_data(data)}
        Ok(None) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
