...
#[utoipa::path(
    post,
    path = "/update",
    operation_id = "erp_sales_return_detail_update",
    request_body(content = UpdateErpSalesReturnDetailRequest, description = "update", content_type = "application/json"),
    responses(
...
#[require_authorize(operation_id = "erp_sales_return_detail_update", authorize = "")]
async fn update(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<UpdateErpSalesReturnDetailRequest>,
) -> CommonResult<()> {
    match service::erp_sales_return_detail::update(&state.db, login_user, payload).await {
