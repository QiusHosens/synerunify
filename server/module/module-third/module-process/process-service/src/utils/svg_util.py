...
def _remove_white_background(img: Image.Image, white_threshold: int = 240) -> Image.Image:
    """
    Remove white background from image, converting colored areas to white points.
    ...
    Returns:
        PIL Image with white background removed (black background, white points for colored areas)
    """
    # Convert to RGB if needed for color detection
    if img.mode != 'RGB':
...
def convert_to_svg_with_bitmap(input_path: str, output_path: str, blacklevel: float = 0.5, scale_down: bool = True, use_edsr: bool = False, remove_white_bg: bool = True, white_threshold: int = 240) -> bool:
    """
    ...
    optionally removes white background, and finally uses potrace Bitmap for high-quality vectorization with curve smoothing.
    ...
    try:
        ...
        # 6. Remove white background if requested
        if remove_white_bg:
            img_final = _remove_white_background(img_final, white_threshold)

        # 7. Convert to grayscale
        img_gray = img_final.convert('L')

        # 8. Create Bitmap object from PIL Image
        # potrace Bitmap constructor accepts PIL Image directly
        bitmap = PotraceBitmap(img_gray, blacklevel=blacklevel)

        # 9. Trace bitmap to get path list
        path_list = bitmap.trace()

        # 10. Build SVG content from paths
        svg_paths = []
        ...
        # 11. Build complete SVG document
...
def convert_image_bytes_to_svg_with_bitmap(image_bytes: bytes, output_path: str, blacklevel: float = 0.5, scale_down: bool = True, use_edsr: bool = False, remove_white_bg: bool = True, white_threshold: int = 240) -> bool:
    """
    Convert image bytes to SVG using potrace Bitmap library.
    
    This method first enlarges the image 4x using high-quality LANCZOS resampling or EDSR model,
    enhances sharpness, then optionally scales back down to original size for better quality,
    optionally removes white background, and finally uses potrace Bitmap for vectorization.
    ...
    try:
        ...
        # 8. Create Bitmap from PIL Image
        bitmap = PotraceBitmap(img_gray, blacklevel=blacklevel)
...
def show_binary_comparison(input_path: str, threshold: int = 127, window_title: str = "Image Comparison", use_edsr: bool = False):
    """
    ...
    try:
        ...
        # 7. Process: remove white background, convert colored areas to white points
        # Convert to RGB if needed for color detection
        ...
        # 8. Create figure with three subplots side by side
        fig, axes = plt.subplots(1, 3, figsize=(24, 8))
        fig.suptitle(window_title, fontsize=14)

        # 9. Display original image on the left
        axes[0].imshow(img_sharp, cmap='gray' if img_sharp.mode == 'L' else None)
        axes[0].set_title(f'Original Image (4x enlarged with {upscale_method} & sharpened)', fontsize=12)
        axes[0].axis('off')

        # 10. Display binary image in the middle
        axes[1].imshow(img_binary, cmap='gray')
        axes[1].set_title(f'Binary Image (threshold={threshold}, 4x enlarged with {upscale_method})', fontsize=12)
        axes[1].axis('off')

        # 11. Display result: white background removed, colored areas as white points
        axes[2].imshow(img_result)
        axes[2].set_title(f'White Background Removed (colored areas → white points)', fontsize=12)
        axes[2].axis('off')

        # 12. Adjust layout and show
        plt.tight_layout()
        plt.show()

        print(f"Displayed image comparison: {input_path} (upscaled with {upscale_method})")
        return True
...
def show_binary_comparison_from_bytes(image_bytes: bytes, threshold: int = 127, window_title: str = "Image Comparison", use_edsr: bool = False):
    """
    ...
    try:
        ...
        print(f"Displayed image comparison from bytes (upscaled with {upscale_method})")
        return True
