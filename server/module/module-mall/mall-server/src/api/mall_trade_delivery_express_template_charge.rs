...
#[utoipa::path(
    post,
    path = "/update",
    operation_id = "mall_trade_delivery_express_template_charge_update",
    request_body(content = UpdateMallTradeDeliveryExpressTemplateChargeRequest, description = "update", content_type = "application/json"),
    responses(
...
#[require_authorize(operation_id = "mall_trade_delivery_express_template_charge_update", authorize = "")]
async fn update(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<UpdateMallTradeDeliveryExpressTemplateChargeRequest>,
) -> CommonResult<()> {
    match service::mall_trade_delivery_express_template_charge::update(&state.db, login_user, payload).await {
