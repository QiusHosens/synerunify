use sea_orm::{Set, NotSet};
use crate::model::mall_trade_after_sale_log::{self, Model as MallTradeAfterSaleLog, ActiveModel as MallTradeAfterSaleLogActiveModel};
use mall_model::request::mall_trade_after_sale_log::{CreateMallTradeAfterSaleLogRequest, UpdateMallTradeAfterSaleLogRequest};
use mall_model::response::mall_trade_after_sale_log::MallTradeAfterSaleLogResponse;

pub fn create_request_to_model(request: &CreateMallTradeAfterSaleLogRequest) -> MallTradeAfterSaleLogActiveModel {
    MallTradeAfterSaleLogActiveModel {
        user_id: Set(request.user_id.clone()),
        user_type: Set(request.user_type.clone()),
        after_sale_id: Set(request.after_sale_id.clone()),
        before_status: request.before_status.as_ref().map_or(NotSet, |before_status| Set(Some(before_status.clone()))),
        after_status: Set(request.after_status.clone()),
        operate_type: Set(request.operate_type.clone()),
        content: Set(request.content.clone()),
        ..Default::default()
    }
}

pub fn update_request_to_model(request: &UpdateMallTradeAfterSaleLogRequest, existing: MallTradeAfterSaleLog) -> MallTradeAfterSaleLogActiveModel {
    let mut active_model: MallTradeAfterSaleLogActiveModel = existing.into();
    if let Some(user_id) = &request.user_id { 
        active_model.user_id = Set(user_id.clone());
    }
    if let Some(user_type) = &request.user_type { 
        active_model.user_type = Set(user_type.clone());
    }
    if let Some(after_sale_id) = &request.after_sale_id { 
        active_model.after_sale_id = Set(after_sale_id.clone());
    }
    if let Some(before_status) = &request.before_status { 
        active_model.before_status = Set(Some(before_status.clone()));
    }
    if let Some(after_status) = &request.after_status { 
        active_model.after_status = Set(after_status.clone());
    }
    if let Some(operate_type) = &request.operate_type { 
        active_model.operate_type = Set(operate_type.clone());
    }
    if let Some(content) = &request.content { 
        active_model.content = Set(content.clone());
    }
    active_model
}

pub fn model_to_response(model: MallTradeAfterSaleLog) -> MallTradeAfterSaleLogResponse {
    MallTradeAfterSaleLogResponse { 
        id: model.id,
        user_id: model.user_id,
        user_type: model.user_type,
        after_sale_id: model.after_sale_id,
        before_status: model.before_status,
        after_status: model.after_status,
        operate_type: model.operate_type,
        content: model.content,
        creator: model.creator,
        create_time: model.create_time,
        updater: model.updater,
        update_time: model.update_time,
    }
}