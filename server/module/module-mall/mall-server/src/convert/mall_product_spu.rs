...
pub fn model_to_page_response(model: MallProductSpu, model_category: Option<MallProductCategoryModel>, model_brand: Option<MallProductBrandModel>, store_names: Vec<String>) -> MallProductSpuPageResponse {
    let category_name = model_category.map(|category| category.name.clone());
    let brand_name = model_brand.map(|brand| brand.name.clone());

    MallProductSpuPageResponse {
        id: model.id,
        name: model.name,
        keyword: model.keyword,
        introduction: model.introduction,
