...
use crate::convert::mall_product_property_value::{create_request_to_model, update_request_to_model, model_to_response};
...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateMallProductPropertyValueRequest) -> Result<()> {
    let mall_product_property_value = MallProductPropertyValueEntity::find_by_id(request.id)
