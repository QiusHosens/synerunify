...
use crate::convert::mall_trade_delivery_express_template_charge::{create_request_to_model, update_request_to_model, model_to_response};
...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateMallTradeDeliveryExpressTemplateChargeRequest) -> Result<()> {
    let mall_trade_delivery_express_template_charge = MallTradeDeliveryExpressTemplateChargeEntity::find_by_id(request.id)
        .filter(Column::TenantId.eq(login_user.tenant_id))
        .one(db)
        .await?
        .ok_or_else(|| anyhow!("记录未找到"))?;

    let mut mall_trade_delivery_express_template_charge = update_request_to_model(&request, mall_trade_delivery_express_template_charge);
    mall_trade_delivery_express_template_charge.updater = Set(Some(login_user.id));
