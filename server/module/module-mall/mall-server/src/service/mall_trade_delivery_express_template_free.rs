...
use crate::convert::mall_trade_delivery_express_template_free::{create_request_to_model, update_request_to_model, model_to_response};
...
pub async fn update(db: &DatabaseConnection, login_user: LoginUserContext, request: UpdateMallTradeDeliveryExpressTemplateFreeRequest) -> Result<()> {
    let mall_trade_delivery_express_template_free = MallTradeDeliveryExpressTemplateFreeEntity::find_by_id(request.id)
        .filter(Column::TenantId.eq(login_user.tenant_id))
        .one(db)
        .await?
        .ok_or_else(|| anyhow!("记录未找到"))?;

    let mut mall_trade_delivery_express_template_free = update_request_to_model(&request, mall_trade_delivery_express_template_free);
    mall_trade_delivery_express_template_free.updater = Set(Some(login_user.id));
...
pub async fn list(db: &DatabaseConnection, login_user: LoginUserContext) -> Result<Vec<MallTradeDeliveryExpressTemplateFreeResponse>> {
    let condition = Condition::all().add(Column::TenantId.eq(login_user.tenant_id));let list = MallTradeDeliveryExpressTemplateFreeEntity::find_active_with_condition(condition)
        .all(db).await?;
