...
use crate::model::system_user::{Model as SystemUserModel, Entity as SystemUserEntity, Column};
...
pub async fn update(db: &DatabaseConnection, request: UpdateSystemUserRequest) -> Result<()> {
...
pub async fn delete(db: &DatabaseConnection, id: i64) -> Result<()> {
    let result = SystemUserEntity::delete_by_id(id).exec(db).await?;
...
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<Option<SystemUserResponse>> {
    let system_user = SystemUserEntity::find_by_id(id).one(db).await?;
    Ok(system_user.map(model_to_response))
}

pub async fn get_paginated(db: &DatabaseConnection, params: PaginatedKeywordRequest) -> Result<PaginatedResponse<SystemUserResponse>> {
...
pub async fn list(db: &DatabaseConnection) -> Result<Vec<SystemUserResponse>> {
    let list = SystemUserEntity::find().all(db).await?;
