...
use crate::services::system_dict::SystemDictService;
use crate::model::system_dict::{self, SystemDict, SystemDictEntity, Column};
use crate::request::system_dict::{CreateSystemDictRequest, UpdateSystemDictRequest, PaginatedKeywordRequest};
...
pub async fn system_dict_route(config: Config) -> Router {
...
async fn update(
    ...
    state.system_dict_service.update(id, payload)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn page(
    ...
    let paginated = state.system_dict_service.get_paginated(params.page, params.page_size)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(paginated))
}
