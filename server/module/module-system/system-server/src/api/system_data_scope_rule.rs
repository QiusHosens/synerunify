...
async fn create(
    ...
    match service::system_data_scope_rule::create(&state.db, payload).await {
...
async fn update(
    ...
    match service::system_data_scope_rule::update(&state.db, payload).await {
...
async fn delete(
    ...
    match service::system_data_scope_rule::delete(&state.db, id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
async fn get_by_id(
    ...
    match service::system_data_scope_rule::get_by_id(&state.db, id).await {
...
async fn page(
    ...
    match service::system_data_scope_rule::get_paginated(&state.db, params).await {
...
async fn list(
    ...
    match service::system_data_scope_rule::list(&state.db).await {
        Ok(data) => {CommonResult::with_data(data)}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}