...
use crate::services::system_data_scope_rule::SystemDataScopeRuleService;
use crate::model::system_data_scope_rule::{self, SystemDataScopeRule, SystemDataScopeRuleEntity, Column};
use crate::request::system_data_scope_rule::{CreateSystemDataScopeRuleRequest, UpdateSystemDataScopeRuleRequest, PaginatedKeywordRequest};
...
async fn update(
    ...
    state.system_data_scope_rule_service.update(id, payload)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn page(
    ...
    let paginated = state.system_data_scope_rule_service.get_paginated(params.page, params.page_size)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(paginated))
}
