...
use crate::services::system_user_role::SystemUserRoleService;
use crate::model::system_user_role::{self, SystemUserRole, SystemUserRoleEntity, Column};
use crate::request::system_user_role::{CreateSystemUserRoleRequest, UpdateSystemUserRoleRequest, PaginatedKeywordRequest};
...
pub async fn system_user_role_route(config: Config) -> Router {
...
async fn update(
    ...
    state.system_user_role_service.update(id, payload)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn page(
    ...
    let paginated = state.system_user_role_service.get_paginated(params.page, params.page_size)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(paginated))
}
