...
use crate::services::system_tenant_package::SystemTenantPackageService;
use crate::model::system_tenant_package::{self, SystemTenantPackage, SystemTenantPackageEntity, Column};
use crate::request::system_tenant_package::{CreateSystemTenantPackageRequest, UpdateSystemTenantPackageRequest, PaginatedKeywordRequest};
...
async fn update(
    ...
    state.system_tenant_package_service.update(id, payload)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn page(
    ...
    let paginated = state.system_tenant_package_service.get_paginated(params.page, params.page_size)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(paginated))
}
