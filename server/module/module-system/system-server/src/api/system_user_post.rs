...
pub async fn system_user_post_router(state: AppState) -> OpenApiRouter {
    OpenApiRouter::new()
...
pub async fn system_user_post_route(state: AppState) -> Router {
    Router::new()
...
#[require_authorize(operation_id = "system_user_post_create", authorize = "")]
async fn create(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<CreateSystemUserPostRequest>,
) -> CommonResult<i64> {
    match service::system_user_post::create(&state.db, login_user, payload).await {
...
#[require_authorize(operation_id = "system_user_post_update", authorize = "")]
async fn update(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<UpdateSystemUserPostRequest>,
) -> CommonResult<()> {
    match service::system_user_post::update(&state.db, login_user, payload).await {
...
#[require_authorize(operation_id = "system_user_post_delete", authorize = "")]
async fn delete(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<()> {
    match service::system_user_post::delete(&state.db, login_user, id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
#[require_authorize(operation_id = "system_user_post_get_by_id", authorize = "")]
async fn get_by_id(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<SystemUserPostResponse> {
    match service::system_user_post::get_by_id(&state.db, login_user, id).await {
...
#[require_authorize(operation_id = "system_user_post_page", authorize = "")]
async fn page(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Query(params): Query<PaginatedKeywordRequest>,
) -> CommonResult<PaginatedResponse<SystemUserPostResponse>> {
    match service::system_user_post::get_paginated(&state.db, login_user, params).await {
...
#[require_authorize(operation_id = "system_user_post_list", authorize = "")]
async fn list(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
