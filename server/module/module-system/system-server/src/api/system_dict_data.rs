...
pub async fn system_dict_data_router(state: AppState) -> OpenApiRouter {
    OpenApiRouter::new()
...
pub async fn system_dict_data_route(state: AppState) -> Router {
    Router::new()
...
#[require_authorize(operation_id = "system_dict_data_create", authorize = "config:dict:add")]
async fn create(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<CreateSystemDictDataRequest>,
) -> CommonResult<i64> {
    match service::system_dict_data::create(&state.db, login_user, payload).await {
...
#[require_authorize(operation_id = "system_dict_data_update", authorize = "config:dict:edit")]
async fn update(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Json(payload): Json<UpdateSystemDictDataRequest>,
) -> CommonResult<()> {
    match service::system_dict_data::update(&state.db, login_user, payload).await {
...
#[require_authorize(operation_id = "system_dict_data_delete", authorize = "config:dict:delete")]
async fn delete(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<()> {
    match service::system_dict_data::delete(&state.db, login_user, id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
#[require_authorize(operation_id = "system_dict_data_get_by_id", authorize = "config:dict:get")]
async fn get_by_id(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<SystemDictDataResponse> {
    match service::system_dict_data::get_by_id(&state.db, login_user, id).await {
...
#[require_authorize(operation_id = "system_dict_data_page", authorize = "config:dict:get")]
async fn page(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
...
async fn list(
    ...
    match service::system_dict_data::list(&state.db, login_user).await {
        Ok(data) => {CommonResult::with_data(data)}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
#[utoipa::path(
    post,
    path = "/enable/{id}",
    operation_id = "system_dict_data_enable",
    params(
        ("id" = i64, Path, description = "id")
    ),
    responses(
...
#[require_authorize(operation_id = "system_dict_data_enable", authorize = "config:dict:enable")]
async fn enable(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<()> {
    match service::system_dict_data::enable(&state.db, login_user, id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
#[utoipa::path(
    post,
    path = "/disable/{id}",
    operation_id = "system_dict_data_disable",
    params(
        ("id" = i64, Path, description = "id")
    ),
    responses(
...
#[require_authorize(operation_id = "system_dict_data_disable", authorize = "config:dict:disable")]
async fn disable(
    State(state): State<AppState>,
    Extension(login_user): Extension<LoginUserContext>,
    Path(id): Path<i64>,
) -> CommonResult<()> {
    match service::system_dict_data::disable(&state.db, login_user, id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}