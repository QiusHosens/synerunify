...
use crate::services::system_menu::SystemMenuService;
use crate::model::system_menu::{self, SystemMenu, SystemMenuEntity, Column};
use crate::request::system_menu::{CreateSystemMenuRequest, UpdateSystemMenuRequest, PaginatedKeywordRequest};
...
async fn update(
    ...
    state.system_menu_service.update(id, payload)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn page(
    ...
    let paginated = state.system_menu_service.get_paginated(params.page, params.page_size)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(paginated))
}
