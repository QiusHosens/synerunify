...
#[require_authorize(operation_id = "system_menu_create", authorize = "")]
async fn create(
    State(state): State<AppState>,
    Json(payload): Json<CreateSystemMenuRequest>,
) -> CommonResult<i64> {
    match service::system_menu::create(&state.db, payload).await {
...
#[require_authorize(operation_id = "system_menu_update", authorize = "")]
async fn update(
    State(state): State<AppState>,
    Json(payload): Json<UpdateSystemMenuRequest>,
) -> CommonResult<()> {
    match service::system_menu::update(&state.db, payload).await {
...
#[require_authorize(operation_id = "system_menu_delete", authorize = "")]
async fn delete(
    State(state): State<AppState>,
    Path(id): Path<i64>,
) -> CommonResult<()> {
    match service::system_menu::delete(&state.db, id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
#[require_authorize(operation_id = "system_menu_get_by_id", authorize = "")]
async fn get_by_id(
    State(state): State<AppState>,
    Path(id): Path<i64>,
) -> CommonResult<SystemMenuResponse> {
    match service::system_menu::get_by_id(&state.db, id).await {
...
#[require_authorize(operation_id = "system_menu_page", authorize = "")]
async fn page(
    State(state): State<AppState>,
    Query(params): Query<PaginatedKeywordRequest>,
) -> CommonResult<PaginatedResponse<SystemMenuResponse>> {
    match service::system_menu::get_paginated(&state.db, params).await {
...
#[require_authorize(operation_id = "system_menu_list", authorize = "")]
async fn list(
    State(state): State<AppState>
) -> CommonResult<Vec<SystemMenuResponse>> {
    match service::system_menu::list(&state.db).await {
        Ok(data) => {CommonResult::with_data(data)}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}