...
pub async fn system_menu_router(db: Arc<DatabaseConnection>) -> OpenApiRouter {
...
async fn create(
    ...
    match state.system_menu_service.create(payload).await {
...
async fn update(
    ...
    match state.system_menu_service.update(payload).await {
...
async fn delete(
    ...
    match state.system_menu_service.delete(id).await {
        Ok(_) => {CommonResult::with_none()}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}
...
async fn get_by_id(
    ...
    match state.system_menu_service.get_by_id(id).await {
...
async fn page(
    ...
    match state.system_menu_service.get_paginated(params).await {
...
async fn list(
    ...
    match state.system_menu_service.list().await {
        Ok(data) => {CommonResult::with_data(data)}
        Err(e) => {CommonResult::with_err(&e.to_string())}
    }
}