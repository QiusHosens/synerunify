...
use crate::services::system_role_menu::SystemRoleMenuService;
use crate::model::system_role_menu::{self, SystemRoleMenu, SystemRoleMenuEntity, Column};
use crate::request::system_role_menu::{CreateSystemRoleMenuRequest, UpdateSystemRoleMenuRequest, PaginatedKeywordRequest};
...
async fn update(
    ...
    state.system_role_menu_service.update(id, payload)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn page(
    ...
    let paginated = state.system_role_menu_service.get_paginated(params.page, params.page_size)
        .await
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(paginated))
}
