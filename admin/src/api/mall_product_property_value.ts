...
const apis = {
  ...
  list_by_ids: "/mall/mall_product_property_value/list_by_ids", // 列表查询
...
export interface MallProductPropertyValueResponse {
  ...
  property_name: string; // 属性项的名字
}

export interface MallProductPropertyValueQueryCondition
  extends PaginatedRequest {}

export const createMallProductPropertyValue = (
  mall_product_property_value: MallProductPropertyValueRequest
): Promise<number> => {
  return api.post<number>(apis.create, mall_product_property_value);
};

export const updateMallProductPropertyValue = (
  mall_product_property_value: MallProductPropertyValueRequest
): Promise<void> => {
...
export const getMallProductPropertyValue = (
  id: number
): Promise<MallProductPropertyValueResponse> => {
  return api.get<MallProductPropertyValueResponse>(`${apis.get}/${id}`);
};

export const listMallProductPropertyValue = (): Promise<
  Array<MallProductPropertyValueResponse>
> => {
...
export const pageMallProductPropertyValue = (
  condition: MallProductPropertyValueQueryCondition
): Promise<PaginatedResponse<MallProductPropertyValueResponse>> => {
  return api.get<PaginatedResponse<MallProductPropertyValueResponse>>(
    apis.page,
    condition
  );
};
