...
export const createMallProductBrand = (
  mall_product_brand: MallProductBrandRequest
): Promise<number> => {
  return api.post<number>(apis.create, mall_product_brand);
};

export const updateMallProductBrand = (
  mall_product_brand: MallProductBrandRequest
): Promise<void> => {
...
export const getMallProductBrand = (
  id: number
): Promise<MallProductBrandResponse> => {
  return api.get<MallProductBrandResponse>(`${apis.get}/${id}`);
};

export const listMallProductBrand = (): Promise<
  Array<MallProductBrandResponse>
> => {
  return api.get<Array<MallProductBrandResponse>>(apis.list);
};

export const pageMallProductBrand = (
  condition: MallProductBrandQueryCondition
): Promise<PaginatedResponse<MallProductBrandResponse>> => {
  return api.get<PaginatedResponse<MallProductBrandResponse>>(
    apis.page,
    condition
  );
};
