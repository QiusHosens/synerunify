...
} from "./mall_product_property_value";
...
const apis = {
  ...
  list_by_ids: "/mall/mall_product_property/list_by_ids", // 列表查询
...
export const createMallProductProperty = (
  mall_product_property: MallProductPropertyRequest
): Promise<number> => {
  return api.post<number>(apis.create, mall_product_property);
};

export const updateMallProductProperty = (
  mall_product_property: MallProductPropertyRequest
): Promise<void> => {
...
export const getMallProductProperty = (
  id: number
): Promise<MallProductPropertyResponse> => {
  return api.get<MallProductPropertyResponse>(`${apis.get}/${id}`);
};

export const getBaseMallProductProperty = (
  id: number
): Promise<MallProductPropertyBaseResponse> => {
  return api.get<MallProductPropertyBaseResponse>(`${apis.get_base}/${id}`);
};

export const listMallProductProperty = (): Promise<
  Array<MallProductPropertyResponse>
> => {
  return api.get<Array<MallProductPropertyResponse>>(apis.list);
};

export const listMallProductPropertyByIds = (
  ids: number[]
): Promise<Array<MallProductPropertyBaseResponse>> => {
  return api.post<Array<MallProductPropertyBaseResponse>>(apis.list_by_ids, {
    ids,
  });
};

export const pageMallProductProperty = (
  condition: MallProductPropertyQueryCondition
): Promise<PaginatedResponse<MallProductPropertyResponse>> => {
  return api.get<PaginatedResponse<MallProductPropertyResponse>>(
    apis.page,
    condition
  );
};
