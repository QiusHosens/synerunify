...
export interface ErpReceiptAttachmentBaseResponse {
  id: number; // 附件ID
  file_id: number; // 文件ID
  remarks: string; // 备注
...
export const createErpReceiptAttachment = (
  erp_receipt_attachment: ErpReceiptAttachmentRequest
): Promise<number> => {
  return api.post<number>(apis.create, erp_receipt_attachment);
};

export const updateErpReceiptAttachment = (
  erp_receipt_attachment: ErpReceiptAttachmentRequest
): Promise<void> => {
...
export const getErpReceiptAttachment = (
  id: number
): Promise<ErpReceiptAttachmentResponse> => {
  return api.get<ErpReceiptAttachmentResponse>(`${apis.get}/${id}`);
};

export const listErpReceiptAttachment = (): Promise<
  Array<ErpReceiptAttachmentResponse>
> => {
  return api.get<Array<ErpReceiptAttachmentResponse>>(apis.list);
};

export const pageErpReceiptAttachment = (
  condition: ErpReceiptAttachmentQueryCondition
): Promise<PaginatedResponse<ErpReceiptAttachmentResponse>> => {
  return api.get<PaginatedResponse<ErpReceiptAttachmentResponse>>(
    apis.page,
    condition
  );
};
