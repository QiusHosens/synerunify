...
import { createOtherErpOutboundOrder, ErpCustomerResponse, ErpOutboundOrderAttachmentRequest, ErpOutboundOrderDetailRequest, ErpOutboundOrderRequest, ErpProductResponse, ErpSettlementAccountResponse, ErpWarehouseResponse, listErpCustomer, listErpProduct, listErpSettlementAccount, listErpWarehouse } from '@/api';
...
const ErpOutboundOrderAdd = forwardRef(({ onSubmit }: ErpOutboundOrderAddProps, ref) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formValues.customer_id && formValues.customer_id != 0) {
      newErrors.customer_id = t('page.erp.sale.outbound.error.customer');
    }
    ...
    formValues.details.forEach((product, index) => {
      if (!product.warehouse_id) {
        newErrors.details[index].warehouse_id = t('page.erp.sale.outbound.detail.error.warehouse');
      }
      if (!product.product_id && product.product_id !== 0) {
        newErrors.details[index].product_id = t('page.erp.sale.outbound.detail.error.product');
      }
      if (!product.quantity) {
        newErrors.details[index].quantity = t('page.erp.sale.outbound.detail.error.quantity');
      }
      if (!product.unit_price && product.unit_price !== 0) {
        newErrors.details[index].unit_price = t('page.erp.sale.outbound.detail.error.unit.price');
      }
      if (!product.tax_rate && product.tax_rate !== 0) {
        newErrors.details[index].tax_rate = t('page.erp.sale.outbound.detail.error.tax.rate');
      }
    });
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      await createOtherErpOutboundOrder(request);
      handleClose();
      onSubmit();
    }
  };
          ...
          <Grid container rowSpacing={2} columnSpacing={4} sx={{ '& .MuiGrid-root': { display: 'flex', justifyContent: 'center', alignItems: 'center' } }}>
            ...
            <Grid size={size}>
              <FormControl sx={{ mt: 2, minWidth: 120, width: '100%' }}>
                <InputLabel required size="small" id="customer-select-label">{t('page.erp.sale.outbound.title.customer')}</InputLabel>
                <Select
                  required
                  size="small"
                  id="customer-helper"
                  ...
                  label={t('page.erp.sale.outbound.title.customer')}
