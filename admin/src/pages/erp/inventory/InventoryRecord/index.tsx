...
import ErpInventoryRecordAdd from './Add';
import ErpInventoryRecordEdit from './Edit';
import ErpInventoryRecordDelete from './Delete';
...
export default function ErpInventoryRecord() {
  ...
  const addErpInventoryRecord = useRef(null);
  const editErpInventoryRecord = useRef(null);
  const deleteErpInventoryRecord = useRef(null);
  ...
  const columns: GridColDef[] = useMemo(
    () => [
      ...
      { field: 'record_type', headerName: t("page."), flex: 1, minWidth: 100 },
      { field: 'record_date', headerName: t("page."), flex: 1, minWidth: 100 },
  ...
  const handleClickOpenAdd = () => {
    (addErpInventoryRecord.current as any).show();
  }

  const handleClickOpenEdit = (erpInventoryRecord: ErpInventoryRecordResponse) => {
    (editErpInventoryRecord.current as any).show(erpInventoryRecord);
  };

  const handleClickOpenDelete = (erpInventoryRecord: ErpInventoryRecordResponse) => {
    (deleteErpInventoryRecord.current as any).show(erpInventoryRecord);
  };
  ...
  return (
    <Box sx={ {height: '100%', display: 'flex', flexDirection: 'column'} }>
      ...
      />
      <ErpInventoryRecordAdd ref={addErpInventoryRecord} onSubmit={refreshData} />
      <ErpInventoryRecordEdit ref={editErpInventoryRecord} onSubmit={refreshData} />
      <ErpInventoryRecordDelete ref={deleteErpInventoryRecord} onSubmit={refreshData} />
    </Box>
  );
}