  ...
  const validateForm = (): boolean => {
    ...
    formValues.details.forEach((product, index) => {
      ...
      if (!product.unit_price && product.unit_price !== 0) {
        newErrors.details[index].unit_price = t('page.erp.purchase.inbound.detail.error.unit.price');
      }
      if (!product.tax_rate && product.tax_rate !== 0) {
        newErrors.details[index].tax_rate = t('page.erp.purchase.inbound.detail.error.tax.rate');
      }
    });
